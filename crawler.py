        ...
        if st.button("Start Spider"):
            ...
            try:
                ...
                if df is not None and not df.empty:
                    ...
                    # Reorder columns: Initial_Status_Type, Indexable, Indexability Reason, then the rest
                    ...
                    def col_idx(name):
                        try:
                            ...
                            return -1
                    order = []
                    for col in ['Initial_Status_Type', 'Indexable', 'Indexability Reason']:
                        if col in cols:
                            order.append(col)
                    for col in cols:
                        if col not in order:
                    ...
                    # Show the DataFrame and download button only
                    st.dataframe(df, use_container_width=True)
