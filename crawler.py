...
class URLChecker:
                ...
                for attempt in range(3):  # Try up to 3 times
                    try:
                        async with self.session.get(url, ssl=False, allow_redirects=True) as resp:
                            ...
                            data["HTTP_Last_Modified"] = resp.headers.get("Last-Modified", "")
    ...
    def parse_html_content(self, data: Dict, content: str, headers: Dict, status_code: int, is_final: bool) -> Dict:
        ...
        try:
            ...
            data["Meta_Description"] = meta_desc["content"].strip() if meta_desc and "content" in meta_desc.attrs else ""
            ...
            data["Meta_Robots"] = meta_robots["content"] if meta_robots and "content" in meta_robots.attrs else ""
            
            # Extract X-Robots-Tag
            data["X_Robots_Tag"] = headers.get("X-Robots-Tag", "")
            
            # Extract HTML lang
            html_tag = soup.find("html")
            data["HTML_Lang"] = html_tag.get("lang", "") if html_tag else ""
            
            # Determine indexability
            data["Is_Indexable"] = "Yes"
            data["Indexability_Reason"] = "Indexable"
            
            # Check for noindex
            if "noindex" in data["Meta_Robots"].lower() or "noindex" in data["X_Robots_Tag"].lower():
...
def main():
    ...
    if mode == "Spider":
        ...
        if st.button("Start Spider"):
            ...
            st.info(f"Starting crawl from: {seed_url}")
            ...
            try:
                ...
                if not results:
                    st.warning("No results from Spider. Check the logs for details.")
                    return
                ...
                if df is not None and not df.empty:
                    ...
                    # Ensure 'Indexable' and 'Indexability Reason' columns exist and are formatted
                    if 'Indexable' in df.columns:
                        df['Indexable'] = df['Indexable'].map(lambda x: 'indexable' if str(x).strip().lower() == 'yes' else 'non-indexable')
                    if 'Indexability Reason' in df.columns:
                        df['Indexability Reason'] = df.apply(lambda row: '' if row.get('Indexable', '').lower() == 'indexable' else row.get('Indexability Reason', ''), axis=1)

                    # Reorder columns: Initial_Status_Type, Indexable, Indexability Reason, then the rest in their original order
                    cols = list(df.columns)
                    main_cols = ['Initial_Status_Type', 'Indexable', 'Indexability Reason']
                    ordered_cols = [col for col in main_cols if col in cols] + [col for col in cols if col not in main_cols]
                    df = df[ordered_cols]

                    # Show only the DataFrame and download button
                    st.dataframe(df, use_container_width=True)
