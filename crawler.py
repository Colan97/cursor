        ...
        if st.button("Start Spider"):
            ...
            try:
                ...
                if df is not None and not df.empty:
                    st.write("Copy and paste directly from the table below into Google Sheets:")
                    st.data_editor(df, use_container_width=True, num_rows="dynamic", disabled=True)
                    st.info("Click the table, press Ctrl+A (Cmd+A on Mac) to select all, then Ctrl+C (Cmd+C) to copy. Paste into Google Sheets. Headers will be included.")
